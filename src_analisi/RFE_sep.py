...
def feature_selection(X_df, X_train, y_train, X_test, task, score, technique):
    ...
    if technique == '5':
        ...
        feature_to_select = mrmr_classif(X=X_task, y=y_task, K=n_features)
        selected_features.extend(feature_to_select)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(out_cv.split(X, y)):
        ...
        for technique in features_technique:
            ...
            if score != 'PUMNS_BulbarSubscore':
                ...
                results_df.to_excel(os.path.join(results_path, 'results_classification_sep_mRMR.xlsx'), index=False)
                ...
                results_df.to_excel(os.path.join(results_path, 'results_regression_sep_mRMR.xlsx'), index=False)
