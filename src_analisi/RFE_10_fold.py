...
def main_classification(df, y, score, task, name_dataset):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(out_cv.split(X, y)):
            ...
            for params in itertools.product(*model_parameters.values()):
                ...
                for technique in features_technique:
                    ...
                    for inner_folder_idx, (train_idx, val_idx) in enumerate(inner_cv.split(X_train, y_train)):
                        ...
                        # Choose the best model based on the differences between the training and validation F1 scores
                        training_f1 = f1_score(y_inner_train, model.predict(X_inner_train))
                        validation_f1 = f1_score(y_inner_val, y_inner_pred)
                        diff_f1 = abs(training_f1 - validation_f1)  # Try to reduce overfitting

                        if diff_f1 < best_f1_validation and training_f1 > 0.85:  # Ensure a reasonable validation F1 score
                            best_f1_validation = validation_f1
                            best_technique = technique
                            best_model = model_class(**params)
                            best_params = params
                            best_features_voted = selected_features
        ...
        results_df.to_excel(os.path.join(results_path, 'results_classification_diff.xlsx'), index=False)
