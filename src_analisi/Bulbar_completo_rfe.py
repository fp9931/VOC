...
def regression(X_train_selected, X_test_selected, y_train, y_test, feature_selection, features, name_file):
    ...
    y_predicted_svm = regression_function(model, parameters, X_train_selected, X_test_selected, y_train, y_test, model_name, feature_selection, features, name_file)
    ...
    y_predicted_rf = regression_function(model, parameters, X_train_selected, X_test_selected, y_train, y_test, model_name, feature_selection, features, name_file)
    ...
    y_predicted_xgb = regression_function(model, parameters, X_train_selected, X_test_selected, y_train, y_test, model_name, feature_selection, features, name_file)
    ...
    y_predicted_knn = regression_function(model, parameters, X_train_selected, X_test_selected, y_train, y_test, model_name, feature_selection, features, name_file)
    ...
    y_predicted_mlp = regression_function(model, parameters, X_train_selected, X_test_selected, y_train, y_test, model_name, feature_selection, features, name_file)
...
def main_regression(df, y, name_file, results):
    ...
    for train_idx, test_idx in train_test_split.split(X, y):
        ...
        for i, id_task in enumerate(task):
            ...
            selected_features_idx = RFE(estimator=LinearRegression(), step=1, n_features_to_select=5).fit(X_task.values, y_task.values).support_
        ...
        feature_selection = "5"
        res_loo = (regression(X_train_selected, X_test_selected, y_train, y_test, feature_selection, features_to_select, name_file))
...
if __name__ == "__main__":
    ...
    main_regression(als_df, y, 'bulbar_LOO_rfe.xlsx', results)
    ...
    als_df = remove_columns(als_df_complete, columns_to_drop)
    main_regression(als_df, y, 'bulbar_noMFCCs_LOO_rfe.xlsx', results)