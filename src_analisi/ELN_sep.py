...
def feature_selection(X_df, X_train, y_train, X_test, task, score, technique):
    ...
    estimator = ElasticNetCV(l1_ratio=[0.1,0.5,0.9,1.0], alphas=None, cv=5, max_iter=5000)
    estimator_5 = ElasticNet(l1_ratio=0.5, alpha=0.1)
    ...
    if technique == '5':
        ...
        for id_task in task:
            ...
            estimator_5.fit(X_task, y_task)

            if hasattr(estimator_5, 'coef_'):
                coefs = np.abs(estimator_5.coef_)

            top_idx = np.argsort(coefs)[-5:]
            selected_features.extend(X_task.columns[top_idx].to_list())
        ...
        estimator.fit(X_task_df, y_task)
        if hasattr(estimator, 'coef_'):
            coefs = np.abs(estimator.coef_)

        top_idx = np.argsort(coefs)[-n_features:]
        selected_features = X_task_df.columns[top_idx].to_list()
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(out_cv.split(X, y)):
        ...
        for technique in features_technique:
            ...
            if score != 'PUMNS_BulbarSubscore':
                ...
                results_df.to_excel(os.path.join(results_path, 'sep_classification_ELN.xlsx'), index=False)
                ...
                results_df.to_excel(os.path.join(results_path, 'sep_regression_ELN.xlsx'), index=False)
