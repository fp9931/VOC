...
#             results_df.to_excel(os.path.join(results_path, f"SEP 2_train.xlsx"), index=False)


results = pd.read_excel(os.path.join(results_path, "results_regression_LOO.xlsx"))
...
for score in scores:
    for dataset in datasets:
        ...
        for i in range(0, len(subset), 3):
            ...
            best_true_values = None
            best_predicted_values = None
            best_predicted_values_rounded = None
            best_rmse_train_rounded = np.inf
            best_rmse_test_rounded = np.inf
            best_r2_train_rounded = np.inf
            best_r2_test_rounded = np.inf

            for index, row in triplet.iterrows():
                if row['RMSE train'] < best_rmse_train:
                    ...
                    best_predicted_values_rounded = row['Predicted rounded']
            ...
            res.append({
                ...
                'Predicted rounded': best_predicted_values_rounded,
                # 'R2 test rounded': best_r2_test_rounded,
            })

            results_df = pd.DataFrame(res)
            results_df.to_excel(os.path.join(results_path, f"LOO_ fig.xlsx"), index=False)
