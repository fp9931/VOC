...
def regression_function(model, parameters, X_train, X_test, y_train, y_test, model_name, feature_selection, features, name_file, results_prediction):
    ...
    results_prediction['Model'].append(model_name)
    results_prediction['Technique'].append(feature_selection)
    results_prediction['Parameters'].append(str(grid_search.best_params_))
    ...
    results_prediction_df.to_excel(os.path.join(results_path, 'results_vocali.xlsx'), index=False)
    return y_predicted
...
if __name__ == "__main__":
    ...
    main_regression(als_df, y, 'bulbar_LOO_vocali_rfe.xlsx', results, results_prediction)
    ...
    als_df = remove_columns(als_df_complete, columns_to_drop)
    main_regression(als_df, y, 'bulbar_noMFCCs_LOO_vocali_rfe.xlsx', results, results_prediction)